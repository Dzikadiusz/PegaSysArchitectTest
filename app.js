...
A. A prescription order page with a list of available prescriptions sent from the customer's health care 
provider. When the user opts to fill one or more prescriptions, the cost summary updates. 
B. A restaurant reservation page in which the user chooses a reservation and the system returns the 
number of available tables. 
C. A purchase order request page with a summary of items. The order total price updates after the 
customer changes a quantity. 
D. A flight booking page where customers can adjust the location and date of the departure and return 
...
Order Summary view? 
A. Convert the Payment section layout that contains order items to a section, and embed this section in 
the Order Summary section. 
B. Build the Order Summary section with a layout inside to group the order items, similar to the Payment 
section. 
C. Embed the Payment section in the Order Summary section. 
D. Reuse the Payment section in the Order Summary view and use a disable when condition to disable 
payment information on the Payment section rule. 
Answer: A 
...
Two.) 
A. Create a check box for customers to indicate whether they want the restaurant to cater the event. Add 
a decision shape that evaluates whether the customer checks the box. 
B. Create a check box for customers to indicate whether they want the restaurant to cater the event. 
Configure the menu preferences and appointment date fields with a visibility condition. 
C. Create a process for providing menu preference and scheduling the menu tasting appointment. 
Create a parallel process for providing the customer with the rental rate quote. 
D. Create a process for customers to indicate menu preference and schedule the menu tasting 
...
A. Configure a layout group to separate each option into individual panels and set the layout to a menu 
format. 
B. Configure a disable condition (when rule) on each field to allow access when the associated option is 
selected on the drop-down control. 
C. Configure a repeating dynamic layout with an embedded section for each option and set the layout
format to grouped. 
...
D. Approval based on Reporting structure and when conditions, using each operator's reporting to 
...
A. An application establishes a goal of four hours to adjust claims for platinum customers. For other 
customers, the application establishes a goal of one business day.
...
D. An application routes upgrade requests for US customers to one work queue, and requests for 
European customers to a different work queue. 
Answer: A D 
...
B. In the Employment Information process add a custom condition to start the process by testing the 
card type. 
...
A. Add a Create Case step that creates the Interview child case. Following this step, add a Wait step that
pauses the parent case until the Interview case reaches a status of Resolved and after 48 hours have 
elapsed. 
B. Add a Create Case step that creates the Interview child case. Following this step, add a Wait step that 
pauses the parent case until 48 hours have elapsed. 
C. Add a Create Case step that creates the Interview child case. Following this step, add a Wait step that 
pauses the parent case until the Interview case reaches a status of Resolved. 
...
A. One team creates a new patch for an application while another team creates a new major version of
the application 
...
C. Create a process using the Send Email step allowing representatives to quickly add the case ID to the 
email. 
D. Use the Insert Property feature of a Send Email step to add the case ID when composing the 
message dialog. 
Answer: D 
...
to 72 hours. 
B. Configure a service level on the stage that contains the Client Consultation process with a deadline of 
72 hours. 
C. Configure a child case to upload recordings. Add a Wait shape with a case dependency until the child 
case is resolved. 
...
C. Add the Update Case shape to the Reservation case type to capture changes to each component 
cost. 
Answer: B 
...
C. Use a circumstance on the check box property to create a rule variant that is effective when the check 
box is selected. 
...
A. Circumstance the existing Open field value record on the application name and update the status 
value to Open-Review. 
B. Add the Open-Review status to the Application record for the application. 
C. Create a field value record named Open-Review for. pyStatusWork with the Apply To class set to the 
class group. 
D. Create a field value record named Open-Review for. pyStatusWork and configure an Access When 
record to apply the status to the correct application
Answer: C 
...
C. A project manager wants to estimate the cost of painting a banquet hall based on labor and materials 
required. 
D. A certification manager wants to determine the number of exams conducted at each testing facility in 
the current quarter. 
Answer: A D 
...
A. Place the Ul rules in the base layer, and create a new layer for the payment rules for both lines of 
business. 
B. Place the Ul rules and generic payment method rules in the base layer, and create a new layer for the 
division-specific payment rules. 
...
A. Developers can configure a Date Time control to display as a text input field, drop-down list, or 
calendar control. 
...
D. Add the report to a public category, and then add the out-of-the-box Reports landing page to the 
portal. 
Answer: D 
...
B. Use a collect information step with an annual salary field. Use a decision shape to determine whether 
to advance to a step to enter cosigner information or complete the process. 
C. Design a user view with an annual salary field. Create an optional action to collect information about 
the cosigner and assign the action to the appropriate stage. 
D. Create an assignment step for a loan officer to review the applicant's annual salary and determine if a 
cosigner is necessary. 
Answer: A B 
...
A. Use a dynamic layout with a responsive breakpoint at 800 pixels to change from inline-grid tripleto 
inline-grid double format. 
...
Two.) 
A. A scholarship eligibility application requires students to enter standardized test scores, Students with 
qualifying test scores can schedule and interview. Students without qualifying test scores receives a 
rejection email. 
B. A catering booking application requires customers to enter information about the expected party size, 
event date, and event time. When customers submit the information the catering company sends a 
confirmation email. 
C. An application requires customer to select the type of request in a drop-down list. The system routes 
the request to the appropriate department work queue. A user with access to the work queue processes 
the case through fulfillment. 
D. A shopping application requires a guest to fill out payment information. A user who enters a 
membership number skips the payment information step. 
Answer: A D 
...
A. Configure the step to continue the parent case when the child cases reach the payment pending 
review status. 
...
D. Configure the step so that users can continue with the parent case process regardless of child case 
status. 
Answer: D 
241.In which two situations is it appropriate to use a decision table? (Choose two.) 
A. A dance studio recommends a membership type based on the number and type of dance classes the 
customer wants to attend every month. 
B. Order total is calculated based on the quantity and price of the items in the customer's shopping cart. 
C. If customers decide to add insurance, checked bags, or early boarding to their airline tickets, extra 
fees are added to their order total. 
D. A hair care company suggests a line of products to customers based on their hair type, scalp 
moisture, hair moisture, and whether it is color-treated. 
Answer: A D 
...
C. Add a weighted condition that evaluates the work status of current and existing cases where work 
status equals "Resolved" and has a weight of "0" 
...
A. The customer cannot complete an application for a checking account until a bank representative 
reviews their credit report. 
...
D. The customer needs to log in to their account before receiving a membership discount on an online 
purchase. 
Answer: C 
...
C. Create two rows, one for the number of bedrooms and one for whether the customer is moving heavy 
furniture. 
D. Create two columns, one for the number of bedrooms and one for whether the customer is moving 
heavy furniture. 
Answer: D 
